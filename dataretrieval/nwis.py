...
def get_qwdata(
    ...
    **kwargs,
    ...
    """
    Get water sample data from qwdata service - RETIRED

    .. warning::

        WARNING: In 2024 the NWIS qw data endpoint stopped
        delivering new data or updates to existing data.
        It has been replaced with the
        [Samples](https://waterdata.usgs.gov/download-samples/#dataProfile=site)
        database.
    ...
    Parameters
    ----------
    None, qw endpoint no longer exists.
    ...
    """
    raise RuntimeError(
        "QW endpoint was retired in 2024, so this function should not"
        " be used. Please try using `get_USGS_samples()` or the"
        " Water Quality Portal functions in the wqp module to pull"
        " discrete water quality samples. If you have questions,"
        " please email CompTools@usgs.gov."
        )
...
def get_info(ssl_check: bool = True, **kwargs) -> Tuple[pd.DataFrame, BaseMetadata]:
    ...
    **kwargs: optional
    ...
    if seriesCatalogOutput in ["True", "TRUE", "true", True]:
        warnings.warn(
            (
                "WARNING: In March 2024, the NWIS qw data endpoint was retired "
                "and no longer receives updates. For more information, "
                "refer to https://doi-usgs.github.io/dataRetrieval/articles/Status.html"
...
def get_record(
    ...
    **kwargs,
    ...
    service: string, default is 'iv'
        ...
        - 'dv' : daily mean data
        - 'qwdata' : discrete samples (RETIRED)
